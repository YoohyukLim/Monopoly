...
public class monoServer {
		...
		private void analysisGameProtocol(GameProtocol data) {
			...
			if (state == GameProtocol.GAME_DOREST) {
				...
				SendToAllPlayers(roomName, data);
					...
					|| state == GameProtocol.GAME_USECARD) {
				SendToAllPlayers(roomName, data);
				...
				if (roomList.get(roomName) != null)
					if (data.getName()
							.equals(roomList.get(roomName).roomMaster))
						gameOver(roomName);
			}
		}
	...
	public synchronized void enterRoom(String RoomName, String name) {
		...
		try {
			if (roomList.get(RoomName).enterRoom(name)) {
				...
				LobbyProtocol room = new LobbyProtocol(name,
				...
				oos.writeObject(new LobbyProtocol(name,
						LobbyProtocol.ENTER_FAIL));
				oos.reset();
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public synchronized void destroyRoom(String RoomName) {
		...
		for (int i = 0; i < clientList.size(); i++) {
			ObjectOutputStream oos = this.clients.get(clientList.get(i))
					.getOuputStream();
			try {
				oos.writeObject(new LobbyProtocol(clientList.get(i),
						LobbyProtocol.EXIT_ROOM));
				oos.reset();
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public void NoticeRoomPlayers(String roomName) {
		...
		for (int i = 0; i < players.size(); i++) {
			ObjectOutputStream oos = clients.get(players.get(i))
					.getOuputStream();
			try {
				oos.writeObject(new LobbyProtocol(players,
						LobbyProtocol.SEND_PLAYER_LIST));
				oos.reset();
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public void SendToAll(Protocol data) {
		...
		while (it.hasNext()) {
			try {
				ObjectOutputStream out = ((ClientManager) clients
						.get((String) it.next())).getOuputStream();
				out.writeObject(data);
				out.reset();
			} catch (IOException e) {
				e.printStackTrace();
